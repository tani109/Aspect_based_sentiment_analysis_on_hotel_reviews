...
path = "E:\\Study_new\\ML_lab\\sentimentAnalysis\\json_data\\check_data"  # declare path to use later for accessing all files by names
...
for i in range(len(files)):
    ...
    with open(path + '\\' + files[i]) as json_file:  # read each json file
        ...
        for reviews in data['Reviews']:
            ...
            overall_ratings.append(reviews.get("Ratings", {}).get("Overall"))  # store overall rating for label
...
print 'accuracy of training set=',accuracy_score(y_train, y_predicted), ' polarity accuracy=', get_polarity_accuracy(y_predicted, y_train)
